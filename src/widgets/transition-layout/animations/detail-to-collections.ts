...
/**
 * Collection detail hero → collections gallery card.
 *
 * nextNode стартует с .pageEnter (opacity: 0).
 * Сначала card → rect hero, потом снимаем pageEnter — без мигания сетки.
 */
export const detailToCollectionsTransition: TransitionAnimation = {
  id: "detail-to-collections",
  ...
  onLeave: ({
    ...
    const collectionsY = historyScroll.get(ROUTES.collections) ?? 0;
    window.__GLOBAL_SCROLL__?.scrollTo(collectionsY, {
      immediate: true,
      force: true,
    });
    ...
    const info = prevNode.querySelector<HTMLElement>(
