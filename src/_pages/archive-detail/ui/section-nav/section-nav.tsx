...
export type SectionNavProps = {
  className?: string;
  items: ArchiveSectionNavItem[];
};

const resolveActiveId = (ids: ArchiveSectionId[]): ArchiveSectionId => {
  const probe = window.innerHeight * 0.35;
  let active: ArchiveSectionId = ids[0] ?? "core";
...
export const SectionNav = (props: SectionNavProps) => {
  ...
  const [activeId, setActiveId] = useState<ArchiveSectionId>(
    ids[0] ?? "core",
  );
